...
class RunStep extends CompositeAction {
    ...
    queueName;
    toStop = false;
    constructor(from = 0, step = 0, limit = 0, to = 0, onBeforeStep, handlerFactory, onAfterStep, errHandler = ErrHandler.RejectAllDone, stepErrHandler = ErrHandler.RejectAllDone) {
        super(errHandler);
        this.from = from;
        this.step = step;
        this.limit = limit;
    ...
    setStepErrHandler(stepErrHandler) {
        this.stepErrHandler = stepErrHandler;
        return this;
    }
    ...
    async doStart(context) {
        ...
        if (err && this.errHandler === ErrHandler.RejectAllDone)
            throw err;
    }
    async doStop(context) {
        if (this.queueAction)
